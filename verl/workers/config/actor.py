...
class ActorConfig(BaseConfig):
    ...
    # Teacher/student context distillation (contextdistillation fork, M4): the independent
    # straight-through gradient scales for the teacher / student path of the local full-vocab
    # forward-KL(teacher‖student). They also carry the KL weight β (a global coefficient folds
    # into both per-path scales, so there is no separate kl_local_coef). Only consumed when
    ...
    # step: θ_ref ← ema_alpha·θ_ref + (1−ema_alpha)·θ_actor (ema_alpha near 1 ⇒ slow drift). This
    # equals SDPO's teacher_update_rate = 1 − ema_alpha (their default 0.05 ⇒ ema_alpha 0.95).
    # false (default) ⇒ the ref stays the frozen θ₀ (byte-for-byte pre-#38). Only meaningful when
    # the ref is actually consumed, i.e. kl_ref_grad_scale != 0 (the M6 ref-anchor KL); the blend
    # is a cheap shard-local param update in the colocated actor_rollout_ref worker.
    ema_ref: bool = False
    # EMA decay for the reference model (issue #38). Only used when ema_ref is true. 1.0 ⇒ ref never
    # moves (== frozen); 0.0 ⇒ ref copies the actor every step. Typical 0.9–0.999 (0.95 = SDPO default).
